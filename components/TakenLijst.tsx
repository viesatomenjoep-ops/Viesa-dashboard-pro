...
export function TakenLijst({
  ...
  return (
    <Kaart>
      ...
      {/* Tom, Joep én Algemeen tegelijk in beeld */}
      <div className="grid gap-4 md:grid-cols-3">
        {TAAK_PERSONEN.map((persoon) => {
          ...
          return (
            <div key={persoon.key} className="rounded-xl border border-navy/10 p-3">
              ...
              {rijen.length === 0 ? (
                <p className="py-2 text-xs text-navy/40">Geen taken.</p>
              ) : (
                        ...
                        <div className="flex items-center gap-2 px-1 py-1.5">
                          ...
                          {t.klant_id && t.klant_naam && (
                            <Link
                              ...
                              className="max-w-[6rem] shrink-0 truncate rounded-full bg-navy/5 px-2 py-0.5 text-[11px] text-navy/60 hover:text-navy"
                            >
                              {t.klant_naam}
                            </Link>
                          )}
      ...
      <p className="mt-3 text-[11px] text-navy/35">Veeg een taak naar links om te verwijderen.</p>
    </Kaart>
  );
}
